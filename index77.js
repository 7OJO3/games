...
client.on('messageCreate', async message => {
    ...
    if (message.content === '!روليت') {
        ...
        const embed = new EmbedBuilder()
            ...
            .setDescription(`عدد المشاركين: 0`);
...
client.on('interactionCreate', async interaction => {
    ...
    if (interaction.customId === 'join') {
        if (!gameParticipants.includes(interaction.user.id)) {
            ...
            const newEmbed = new EmbedBuilder()
                .setImage('https://cdn.discordapp.com/attachments/1501300022808023351/1524396477940764762/IMG_8626.png?ex=6a4f985a&is=6a4e46da&hm=60da20c229ef96b6f3a1fc38d911a81125eb0ab3e1386fe35d7ab2b2f293dbc6&')
                .setDescription(`عدد المشاركين: ${gameParticipants.length}`);
    ...
    if (interaction.customId === 'shop') {
        ...
        const shopEmbed = new EmbedBuilder()
            .setTitle('متجر الأغراض')
            .setDescription('مرحباً بك في المتجر، اختر غرضاً (التكلفة 3 نقاط):')
            .setImage('https://discord.com/channels/1009291746410254337/1501300022808023351/1524396222184423474');
...
async function startMafiaGame(channel) {
    ...
    game.players.forEach((p, i) => {
        p.role = roles[i];
        client.users.send(p.id, { content: `دورك: ${p.role}`, files: [IMAGES[p.role.toUpperCase()]] });
    });
